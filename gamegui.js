...
$('#undoButton').click(function()
	{
		...
		if(lastAction == TRAPDOOR)
			$($('.createDoor:hidden')[0]).show();
		if(lastAction == FLAMETHROWER)
			$($('.createFire:hidden')[0]).show();
		if(lastAction == WAYPOINT)
			$($('.createWaypoint:hidden')[0]).show();
		if(lastAction == LASER)
			$($('.createLaser:hidden')[0]).show();
		if(lastAction == ENEMY)
			$($('.createEnemy:hidden')[0]).show();
...
for(var i =0; i < NUM_FLAMETHROWERS; i++)
	$('#gameEditGUI').append('<div class="createFire"  draggable="true" style="display:inline-block;width: 50px;height: 50px;background: red;color:white;margin: auto;">Flame Thrower</div>');

for(var i =0; i < NUM_TRAPDOORS; i++)
	$('#gameEditGUI').append('<div class="createDoor"  draggable="true" style="display:inline-block;width: 50px;height: 50px;background: brown;color:white;margin: auto;">Trap Door</div>');

for(var i =0; i < NUM_WAYPOINTS; i++)
	$('#gameEditGUI').append('<div class="createWaypoint"  draggable="true" style="display:inline-block;width: 50px;height: 50px;background: lightblue;color:white;margin: auto;">Way Point</div>');

for(var i =0; i < NUM_LASERS; i++)
	$('#gameEditGUI').append('<div class="createLaser"  draggable="true" style="display:inline-block;width: 50px;height: 50px;background: purple;color:white;margin: auto;">Laser</div>');

for(var i =0; i < NUM_ENEMIES; i++)
	$('#gameEditGUI').append('<div class="createEnemy"  draggable="true" style="display:inline-block;width: 50px;height: 50px;background: black;color:white;margin: auto;">Enemy</div>');
...
$(document).on('setstatecomplete')
{
		...
		for(var i in objects)
		{
			...
			{
				$($('.createDoor:visible')[0]).hide();
			}
			if(gameType == 'FlameThrower') 
			{
				$($('.createFire:visible')[0]).hide();
			}
			if(gameType == 'WayPoint') 
			{
				$($('.createWaypoint:visible')[0]).hide();
			}
			if(gameType == 'Laser') 
			{
				$($('.createLaser:visible')[0]).hide();
			}
			if(gameType == 'Enemy') 
			{
				$($('.createEnemy:visible')[0]).hide();
			}
